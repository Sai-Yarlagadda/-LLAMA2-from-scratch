...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        dk = self.head_dim
        scaled_dot_product = qk/math.sqrt(dk)
        sdp_attention = F.softmax(scaled_dot_product, dim = -1)
        attention_dropout = self.attn_dropout(sdp_attention)
        output = torch.matmul(attention_dropout, value)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        3) a residual connection (i.e., add the input to the output of the self-attention)  ----> why is this residual connection needed?
        ...
        self_attention_output = self.attention(x_norm)
        x = x + self_attention_output #add & norm layer transformer
        output_normalization = self.ffn_norm(x)
        feedforward_output = self.feed_forward(output_normalization) # add & norm - feedforward
        feedforward_output = feedforward_output + x
